...
__global__ void sor_rb_kernel_r(double* d, CellType* d_types, double* d_faces_k, double* d_total_ks, unsigned char* d_active, int rows, int cols, int depth, double omega) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (d_types[(k * rows * cols) + i * cols + j] == INTERIOR && (i + j + k) % 2 == 0){
        double oldPoint = d[(k * rows * cols) + i * cols + j];
        double avg = update_cell(d, d_faces_k, d_total_ks, d_active, i, j, k, rows, cols, depth);
        double delta = omega * (avg - oldPoint);
        d[(k * rows * cols) + i * cols + j] = oldPoint + delta;
    }
}
  
__global__ void sor_rb_kernel_b(double* d, CellType* d_types, double* d_faces_k, double* d_total_ks, unsigned char* d_active, int rows, int cols, int depth, double omega) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= (rows - 2) * (cols - 2) * (depth - 2)) return;
    ...
    if (d_types[(k * rows * cols) + i * cols + j] == INTERIOR && (i + j + k) % 2 == 1){
        double oldPoint = d[(k * rows * cols) + i * cols + j];
        double avg = update_cell(d, d_faces_k, d_total_ks, d_active, i, j, k, rows, cols, depth);
        double delta = omega * (avg - oldPoint);
        d[(k * rows * cols) + i * cols + j] = oldPoint + delta;
