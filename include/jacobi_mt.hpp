...
void sweep_rows(const Grid3D& g, std::vector<double>& cur, std::vector<double>& nxt, int r_start, int r_end, double& localMax){
    localMax = 0;
    double change = 0;
...
void worker_job_3d(Grid3D& g, std::vector<double>& cur, std::vector<double>& nxt, int r_start, int r_end, double& localMax, int max_iter, std::barrier<std::function<void()>>& sync_point, std::atomic<bool>& converged){
...
int jacobi_mt_solve(Grid3D& g, double tol, int max_iter, int num_threads){
    ...
    for (int t = 0; t < num_threads; t++){
        ...
        workers.push_back(std::thread(worker_job_3d, std::ref(g), std::ref(cur), std::ref(nxt), r_start, r_end, std::ref(localMaxes[t]), max_iter, std::ref(sync_point), std::ref(converged)));
    }
    for (auto& w : workers) w.join();
