...
int main(){
    ...
    cudaMemcpy(d_faces_k, g.getFacesKPtr(), 4 * rows * cols * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_total_ks, g.getTotalKPtr(), rows * cols * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_active, g.getActivePtr(), rows * cols * sizeof(unsigned char), cudaMemcpyHostToDevice);
