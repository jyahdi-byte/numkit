...
int main(){
    ...
    for (int i = 0; i < sweeps; i++){
        sor_rb_kernel_r<<<numBlocks, threadsPerBlock>>>(d, d_types, d_faces_k, d_total_ks, d_active, rows, cols, omega);
        CUDA_CHECK(cudaDeviceSynchronize());
        sor_rb_kernel_b<<<numBlocks, threadsPerBlock>>>(d, d_types, d_faces_k, d_total_ks, d_active, rows, cols, omega);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
